...
def build_model(input_shape, preprocess_fn=None, bce_loss_weight=0.1, half_res=True):
    ...
    # Remove the last singleton dimension so that outputs become (None, 256, 256)
    dmp_squeezed = layers.Lambda(lambda x: tf.squeeze(x, axis=-1), name='dmp_squeeze')(dmp_upsampled)
    amp_squeezed = layers.Lambda(lambda x: tf.squeeze(x, axis=-1), name='amp_squeeze')(amp_upsampled)
